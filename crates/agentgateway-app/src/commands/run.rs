...
pub(crate) fn execute(args: RunArgs) -> anyhow::Result<()> {
	...
	tokio::runtime::Builder::new_current_thread()
		...
		.block_on(async move {
			...
			let mut config = agentgateway::runtime::config::parse_config(contents, local_config_source)?;
...
async fn validate(
	...
	let config = agentgateway::runtime::config::parse_config(contents, local_config_source)?;
