...
impl Store {
	...
	/// Called when the local self-identity is initialized.
	...
	fn remove_workload_for_insert(&mut self, uid: &Strng) {
		if let Some(previous) = self.workloads.remove(uid) {
...
pub struct ServiceStore {
	...
	/// Allows for lookup of services by network address.
	pub(super) by_vip: HashMap<NetworkAddress, Arc<Service>>,

	/// Allows for lookup of services by hostname, and then by namespace. In most cases, there will be a single
...
impl ServiceStore {
	fn insert_endpoint_for_services(
		...
		services: &HashMap<String, HashMap<u16, u16>>,
...
impl StoreUpdater {
	...
	pub fn sync_local(
		...
		for wl in workloads {
			...
			s.services
				.insert_endpoint_for_services(&w, &wl.services, source.as_ref())?;
