...
pub struct Relay {
	...
	pub(crate) security_pipeline: Option<Arc<gateway_adapter::SecurityPipelineConfig>>,
	pub(crate) security_audit: Option<Arc<security_integration_agentgateway::SecurityConfig>>,
...
pub struct RelayInputs {
	...
	pub security_pipeline: Option<Arc<gateway_adapter::SecurityPipelineConfig>>,
	pub security_audit: Option<Arc<security_integration_agentgateway::SecurityConfig>>,
...
impl Relay {
	...
	pub fn with_policies(&self, policies: McpAuthorizationSet) -> Self {
		Self {
			...
			security_audit: self.security_audit.clone(),
	...
	fn enforce_security_pipeline(
		&self,
		r: &JsonRpcRequest<ClientRequest>,
		ctx: &IncomingRequestContext,
	) -> Result<(), UpstreamError> {
		let Some(config) = self.security_pipeline.as_deref() else {
			return Ok(());
		};
		crate::mcp::security::enforce_tool_call(config, r, ctx)
	}
	...
	fn audit_security_pipeline(
		...
		if let Some(config) = self.security_audit.as_deref() {
			crate::mcp::security::audit_request(config, r, ctx, mcp_server_id)?;
		}
		Ok(())
	}
	...
	pub fn merge_tools(
		&self,
		request_id: String,
		identity: gateway_adapter::GatewayIdentity,
	) -> Box<MergeFn> {
		let policies = self.policies.clone();
		let security = self.security_audit.clone();
		let default_target_name = self.upstreams.default_target_name.clone();
			...
			let tools = streams
				...
				.flat_map(|(server_name, s)| {
					...
					tools
						...
						// S2: enforce mode exposes only tools the caller may invoke. Audit and
						// shadow retain the complete discovery response while recording decisions.
						.filter(|t| {
							security.as_ref().is_none_or(|security| {
								crate::security::evaluate(
									security,
									&gateway_adapter::tool_invoke_for_identity(
										format!("{request_id}:{}", t.name),
										identity.clone(),
										t.name.to_string(),
									),
								)
