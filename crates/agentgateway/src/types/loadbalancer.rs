...
pub struct EndpointSet<T> {
	...
	// action_mutex serializes all endpoint mutations, rebalancing, and the eviction worker.
	...
	fn worker(
		...
		tokio::task::spawn(async move {
			...
			let handle_eviction = |uneviction_heap: &mut BinaryHeap<UnevictEntry>| {
				...
				trace!(%key, "unevict");
				// Serialize against endpoint changes and rebalancing — without this, their load→store
...
mod tests {
	...
	fn update_sampler_preserves_uniform_when_cap_matches() {
		// Common case: all endpoints share the default cap=1. Adds and
